// use cases regexTransform function
// regex generated by copilot
//  - italic using asterisk:         tag = "em"      regex = '/\*(.*?)\*/g'
//  - bold   using asterisk:         tag = "strong"  regex = /\*\*(.*?)\*\*/g
//  - strikethrough using 2 tildes:  tag = "del"     regex = '/~~(.*?)~~/g'
//  - code   using 2 backticks:      tag = "code"    regex = '/``(.*?)``/g'

// based on https://davidwells.io/snippets/regex-match-markdown-links
//  - link   using brackets:         tag = "a"       regex = '/\[([\w\s\d]+)\]\((https?:\/\/[\w\d./?=#]+)\)/;'

const regexTransform = (regex, tag, text) =>
  text.replace(regex, `<${tag}>$1</${tag}>`);

const italicRegexTransform = (text) => regexTransform(/\*(.*?)\*/g, "em", text);

const boldRegexTransform = (text) =>
  regexTransform(/\*\*(.*?)\*\*/g, "strong", text);

const strikethroughRegexTransform = (text) =>
  regexTransform(/~~(.*?)~~/g, "del", text);

const linkRegexTransform = (text) =>
  text.replace(
    /\[([\w\s\d]+)\]\((https?:\/\/[\w\d./?=#]+)\)/,
    `<a href="$2">$1</a>`
  );

const codeTransform = (text) => {
  return text
    .split("``")
    .map((line, index) => {
      return index % 2 === 1
        ? `<code>${line}</code>`.replace(/<p>/g, "").replace(/<\/p>/g, "")
        : line.replace(/`([^`]*)`/g, "<code>$1</code>");
    })
    .join("");
};

const splitBlockquotes = text => text.split('\n').reduce((acc, line) => {
  line[0] === ">"
    ? acc[acc.length - 1].every(l => l.startsWith('>'))
      ? acc[acc.length - 1].push(line)
      : acc.push([line])
    : acc.push([line])
  return acc;
}, [[]]);

const cleanBlockQuotes = (blocks) => {
  return blocks.map(block => block.length > 1 ? block.map((line, index) => {
    return index !== 0 && index !== block.length - 1 ? line.replace(/>/g, "") : line;
  }) : block);
}

const addBlockQuoteTags = (blocks) => {
  blocks.slice(1).forEach(block => block[0][0] === ">" ? block.unshift("<blockquote>") && block.push("</blockquote>") : block);
  return blocks.slice(1);
}

const joinBlocks = (blocks) => {
  return blocks.map(block => block.join("")).join("\n");
}

// regex generated by copilot
const replaceUnorderedListItems = (text) =>
  text.replace(/^\s*\*\s(.*)/gm, "<ul><li>$1</li></ul>");

const replaceOrderedListItems = (text) =>
  text.replace(/^\s*\d+\.\s(.*)/gm, "<ol><li>$1</li></ol>");

const mergeSameUnorderedListItems = (text) =>
  text.replace(/<\/li><\/ul>\n<ul><li>/g, "</li><li>");

const mergeSameOrderedListItems = (text) =>
  text.replace(/<\/li><\/ol>\n<ol><li>/g, "</li><li>");

const fixNestingForOrderedListInUnorderedList = (text) =>
  text
    .replace(/<\/li><\/ul>\n<ol>/g, "<ol>")
    .replace(/<\/ol>\n<ul><li>/g, "</ol></li><li>");

const compose =
  (...funcs) =>
  (text) =>
    funcs.reduceRight((acc, func) => func(acc), text);

export const blockquoteTransform = compose(joinBlocks, cleanBlockQuotes, addBlockQuoteTags, splitBlockquotes);

export const formatTransform = compose(
  codeTransform,
  linkRegexTransform,
  italicRegexTransform,
  boldRegexTransform,
  strikethroughRegexTransform,
);

const replaceListCompose = compose(
  replaceOrderedListItems,
  replaceUnorderedListItems
);
const mergeListCompose = compose(
  mergeSameOrderedListItems,
  mergeSameUnorderedListItems
);
const listTransform = compose(mergeListCompose, replaceListCompose);
export const listRegexTransform = compose(
  fixNestingForOrderedListInUnorderedList,
  listTransform
);
